import React from 'react'

const PasswordOverVier = () => {
    return (
        <div className='rounded-lg shadow-lg  w-2/6 py-5 hover:scale-95 duration-300 mt-10 border border-gray-300 ml-10 px-5 '>

            <div className='rounded-tr-md rounded-tl-md p-4 border-zinc-950 border hover:bg-zinc-950 text-xl hover:text-white duration-300 text-center '>Password Overview</div>
            <div className='flex justify-around mt-1 border-t border-b py-2 items-center justify-items-center hover:bg-zinc-950 duration-300 hover:text-white'>
                <div className='flex justify-start w-full'>
                    <div className='mx-4'>facebook</div>
                </div>
                <div className='flex justify-around w-full'>
                    < div className='flex'>
                        <div> ............</div>
                        <div className='mx-2'>see</div>
                    </div>
                    <div>:</div>
                </div>
            </div>
            <div className='flex justify-around mt-1 border-t border-b py-2 items-center justify-items-center hover:bg-zinc-950 duration-300 hover:text-white'>
                <div className='flex justify-start w-full'>
                    <div className='mx-4'>facebook</div>
                </div>
                <div className='flex justify-around w-full'>
                    < div className='flex'>
                        <div> ............</div>
                        <div className='mx-2'>see</div>
                    </div>
                    <div>:</div>
                </div>
            </div>
            <div className='flex justify-around mt-1 border-t border-b py-2 items-center justify-items-center hover:bg-zinc-950 duration-300 hover:text-white'>
                <div className='flex justify-start w-full'>
                    <div className='mx-4'>facebook</div>
                </div>
                <div className='flex justify-around w-full'>
                    < div className='flex'>
                        <div> ............</div>
                        <div className='mx-2'>see</div>
                    </div>
                    <div>:</div>
                </div>
            </div>
            <div className='flex justify-around mt-1 border-t border-b py-2 items-center justify-items-center hover:bg-zinc-950 duration-300 hover:text-white'>
                <div className='flex justify-start w-full'>
                    <div className='mx-4'>facebook</div>
                </div>
                <div className='flex justify-around w-full'>
                    < div className='flex'>
                        <div> ............</div>
                        <div className='mx-2'>see</div>
                    </div>
                    <div>:</div>
                </div>
            </div>
            <div className='flex justify-around mt-1 border-t border-b py-2 items-center justify-items-center hover:bg-zinc-950 duration-300 hover:text-white'>
                <div className='flex justify-start w-full'>
                    <div className='mx-4'>facebook</div>
                </div>
                <div className='flex justify-around w-full'>
                    < div className='flex'>
                        <div> ............</div>
                        <div className='mx-2'>see</div>
                    </div>
                    <div>:</div>
                </div>
            </div>
            <div className='flex justify-around mt-1 border-t border-b py-2 items-center justify-items-center hover:bg-zinc-950 duration-300 hover:text-white'>
                <div className='flex justify-start w-full'>
                    <div className='mx-4'>facebook</div>
                </div>
                <div className='flex justify-around w-full'>
                    < div className='flex'>
                        <div> ............</div>
                        <div className='mx-2'>see</div>
                    </div>
                    <div>:</div>
                </div>
            </div>
            

        </div>

    )
}

export default PasswordOverVier
